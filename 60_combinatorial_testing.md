...
The number of possible permutations of a set is equal to the **factorial** of _r_, or `r!` \(pronounced "r factorial"\), or `r * (r - 1) * (r -2 ) ... * 1`. For example, the factorial of 3 is `3 * 2 * 1 = 6`, and the factorial of 5 is `5 * 4 * 3 * 2 * 1 = 120`.
...
As you may have already guessed from the name, when we discuss combinatorial testing, we will be interested in checking **combinations** of a set of elements, not permutations. A combination is some _unordered_ subset of a set. Using similar nomenclature as with permutations, we can also refer to an **r-combination** of a set to indicate combinations of subsets. Ordinarily, we are interested in combinations of a smaller size than the original set, as an _r_-combination of a set where _r_ is the same size of the set is just the set itself.

The number of _r_-combinations of a given set of size _n_ is called the **binomial coefficient**. It is determined by the following formula:
